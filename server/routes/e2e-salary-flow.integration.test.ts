...
describe('e2e salary flow', () => {
  it('creates attendance with employeeId and salary record preserves the chain', async () => {
    ...
    try {
      ...
      // Step 1: Create attendance for employee ID=2 (陳文山)
      ...
      // Step 2: Create attendance for employee ID=8 (謝琮閔)
      ...
      expect((attList.body as unknown[]).length).toBe(2);
  ...
  it('salary calculation receives the correct employeeId (not fallback 1)', async () => {
  ...
  it('multi-employee attendance records maintain separate employeeIds', async () => {
    ...
    try {
      ...
      expect(state.salaryRecords[0]).toMatchObject({ employeeId: 2, employeeName: '陳文山' });
      expect(state.salaryRecords[1]).toMatchObject({ employeeId: 8, employeeName: '謝琮閔' });
      ...
      expect((listResult.body as unknown[]).length).toBe(2);
    } finally {
      await server.close();
    }
  });
