...
describe('aes migration guard', () => {
  it('detects remote database targets and preserves local ones', () => {
    ...
    expect(
      describeDatabaseTarget('postgresql://user:pass@aws-0-ap-northeast-1.pooler.supabase.com:6543/postgres')
    ).toEqual({
  ...
  it('blocks execute when remote approval or execute prerequisites are missing', () => {
    expect(() =>
      validateAesMutationRequest({
        mode: 'execute',
        databaseUrl: 'postgresql://user:pass@aws-0-ap-northeast-1.pooler.supabase.com:6543/postgres',
        allowRemote: false,
        operator: 'Marcus',
