...
3. Runtime install: `npm ci --omit=dev`
4. Root `.npmrc` fallback: `include=dev`
5. Rollup native fallback: `node scripts/ensure-rollup-native.mjs`
...
4. Clear the Zeabur build cache if the logs still reflect an older install strategy
